...
fn setup_ui(mut commands: Commands, materials: Res<Materials>, font_spec: Res<FontSpec>) {
            ...
            Interaction::Clicked => {
                *material = button_materials.pressed.clone();
                ...
                match run_state.current() {
                    ...
                    RunState::GameOver => run_state.set(RunState::Playing).unwrap(),
                }
            }
            Interaction::Hovered => {
...
fn button_text_system(
    ...
    let children = button_query.single().expect("expected only one button");
